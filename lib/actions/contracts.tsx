...
export async function changeContractStatus(
  ...
  if (!canTransitionContract(contract.statut, newStatus)) {
    throw new Error(`Transition de "${contract.statut}" vers "${newStatus}" non autorisée.`);
  }
...
export async function generateContractPdf(contractId: string): Promise<string> {
  ...
  if (!customer || !unit || !company) throw new Error("Données manquantes pour générer le PDF.");
