...
// Page de preuve de signature électronique, réutilisée pour le contrat et le
// mandat SEPA — un même geste de signature peut certifier plusieurs documents,
// chacun avec sa propre empreinte SHA-256 mais la même identité de signataire.
export function SignatureProofPage({ proof }: { proof: SignatureProof }) {
  return (
    <Page size="A4" style={styles.page}>
      <Text style={styles.title}>Preuve de signature électronique — {proof.documentLabel}</Text>
      ...
      <View style={styles.section}>
        ...
        {(proof.extraFields ?? []).map((field) => (
          <View style={styles.row} key={field.label}>
            <Text style={styles.label}>{field.label}</Text>
            <Text style={styles.mono}>{field.value}</Text>
          </View>
        ))}
      </View>

      <Text style={styles.legal}>
        Ce document atteste que le signataire ci-dessus a pris connaissance du document présenté à la date et
        à l&apos;heure indiquées, a coché une case de consentement explicite et a confirmé son identité en
        saisissant son nom complet. L&apos;empreinte SHA-256 ci-dessus permet de vérifier que le document
